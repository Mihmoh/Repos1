...
void menu(students*, students*);		//функции
int checkInput(int);
void continuationCondition();
students* creator(students*, students**);
...
students* downloadText(students*, students**);
void creatorBinFile(students*);
void recursBINFile(students*, FILE*);
students* downloadBIN(students*, students**);
...
int main()
{
	...
	students* pointerTail = nullptr;
	menu(pointHead, pointerTail);
}
...
students* creator(students* pointerHead, students** pointerTail)		//создание стека
{
...
students* downloadText(students* pointerHead, students** pointerTail) //функция загрузки текстового файла
{
	pointerHead = nullptr;
	char* name;
	students* pointerPrevious = nullptr, * pointerNEXT;
	char checker;
	...
	while (!feof(file)) {
		...
		fscanf(file, "%c", &checker);
		pointerNEXT->pointerNext = pointerPrevious;
		pointerNEXT = pointerPrevious;
		pointerPrevious = nullptr;
	}
	pointerNEXT->pointerNext = nullptr;
	*pointerTail = pointerNEXT;
	fclose(file);
	return pointerHead;
